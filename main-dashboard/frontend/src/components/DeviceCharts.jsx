...
export default function DeviceCharts({ vanId }) {
  ...
  useEffect(() => {
    ...
    const handleDevices = (payload) => {
      if (!mounted || payload.vanId !== vanId) return;
  ...
  if (loading) {
    return <div className="text-gray-500 text-sm">Loading devices...</div>;
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
        {devices.map((d) => {
          ...
          return (
            <div key={`device-${monId}`} className="bg-white p-3 rounded border">
      ...
      {devices.map((d) => {
        const monId = d.monitorId || d.monitor_id;
        const chartData = history[monId] || [];
        if (chartData.length === 0) return null;
        
        return (
          <div key={`chart-${monId}-${chartData.length}`} className="bg-white p-4 rounded border">
            <div className="font-medium mb-2">{d.name} - Latency Trend ({chartData.length} points)</div>
