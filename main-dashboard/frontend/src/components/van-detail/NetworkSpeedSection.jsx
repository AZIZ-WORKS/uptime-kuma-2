...
export default function NetworkSpeedSection({ vanId }) {
  ...
  useEffect(() => {
    ...
    fetchSpeedtestData();
    const interval = setInterval(fetchSpeedtestData, 30000); // Refresh every 30 seconds
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-blue-50 rounded-lg p-4 border-2 border-blue-200">
          ...
          <div className="text-2xl font-bold text-blue-600">
            {currentStats.download !== null ? `${currentStats.download.toFixed(2)} Mbps` : 'N/A'}
          </div>
        </div>
        <div className="bg-green-50 rounded-lg p-4 border-2 border-green-200">
          <div className="text-sm text-gray-600 mb-1">Current Upload</div>
          <div className="text-2xl font-bold text-green-600">
            {currentStats.upload !== null ? `${currentStats.upload.toFixed(2)} Mbps` : 'N/A'}
          </div>
        </div>
        <div className="bg-purple-50 rounded-lg p-4 border-2 border-purple-200">
          <div className="text-sm text-gray-600 mb-1">Current Ping</div>
          <div className="text-2xl font-bold text-purple-600">
            {currentStats.ping !== null ? `${currentStats.ping.toFixed(0)} ms` : 'N/A'}
          </div>
        </div>
      </div>
